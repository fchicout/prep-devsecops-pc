...
def test_docker_apt_repository_configured(host):
    ...
    assert repo_file.contains("https://download.docker.com/linux/ubuntu")
    assert repo_file.contains("[signed-by=/etc/apt/keyrings/docker.gpg]")
...
def test_docker_daemon_is_accessible(host):
    ...
    assert cmd.rc == 0, "'docker info' should run successfully"
